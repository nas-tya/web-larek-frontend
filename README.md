...
**Класс Model** - базовый класс для моделей, в нём всего один метод - emitChanges, который сообщает о том, что модель была изменена.

**Класс Product** - отвечает за хранение информации о товаре. С его помощью создаются товары.

**Класс AppState** - отслеживает общее состояние приложения, включая каталог товаров, содержимое корзины и информацию о заказе. Инициирует события при изменении состояния приложения.

***Методы:***
- *addToBasket(itemId: IProductItem):* добавление товара в корзину.
- *deleteFromBasket(itemId: IProductItem):* удаление товара из корзины.
...
**Класс Component** - базовый класс, упрощает управление содержимым и состоянием элементов.
...
***Класс Form*** - обеспечивает функциональность управления формой, обработку ввода пользователей, валидацию и отправку данных.
...
***Класс Modal*** - обеспечивает функциональность открытия и закрытия модального окна и управление его содержимым.
...
***Класс Success*** - компонент для отображения сообщения об успешном оформлении заказа.
 
***Класс Card*** - компонент для отображения карточки товара на странице.
...
***Класс CardInBasket*** - компонент для отображения карточки товара в коризне.

***Методы:***
- *id:* геттер и сеттер для установки и получения идентификатора карточки.
- *title:* сеттер для установки и получения заголовка карточки.
- *price:* сеттер для установки и получения цены карточки.

***Класс OrderContacts*** - компонент для отображения формы с контактами покупателя.

***Методы:***
- *phone:* сеттер для установки телефона пользователя.
- *email:* сеттер для установки почты пользователя.

***Класс OrderAddress*** - компонент для отображения формы с адресом покупателя и методомоплаты.
...
***Класс Page*** - компонент для отображения и управления основными элементами страницы.
...
Когда юзер открывает страинцу ларька, происходит изменение элементов каталога товаров (items:changed), приложение обновляет отображение каталога на странице.
